...
const validationSchema = yup.object({
  no_ktp: yup.string().required("Harus Diisi"),
});

const useHandleSubmitMutation = (user) => {
  return useMutation(async (formPayload) => {
    return axios
      .put(`/api/simpeg/${user.id}/nomor`, formPayload)
      .then((res) => res.data)
      .catch((err) => {
...
const handleSubmit = (values, setSubmitting, mutate, queryClient, user) => {
  mutate(values, {
    onSuccess: (response) => {
      ...
      queryClient.invalidateQueries(["user", user.id, "nomor"]);
    },
    onError: (error) => {
...
export default SimpegFormNomor;
