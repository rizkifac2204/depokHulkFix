...
const useHandleSubmitMutation = (user) => {
  return useMutation(async (formPayload) => {
    return axios
      .put(`/api/simpeg/${user.id}/umum`, formPayload)
      .then((res) => res.data)
      .catch((err) => {
...
const handleSubmit = (values, setSubmitting, mutate, queryClient, user) => {
  mutate(values, {
    onSuccess: (response) => {
      ...
      queryClient.invalidateQueries(["user", user.id, "umum"]);
    },
    onError: (error) => {
...
function SimpegFormUmum({ user }) {
  ...
  const {
    ...
    queryFn: ({ signal }) =>
      axios
        .get(`/api/simpeg/${user.id}/umum`, { signal })
        .then((res) => res.data)
        .catch((err) => {
