...
const useHandleSubmitMutation = (user) => {
  return useMutation(async (formPayload) => {
    try {
      const res = await axios.put(`/api/simpeg/${user.id}/alamat`, formPayload);
      return res.data;
    } catch (err) {
...
const handleSubmit = (values, setSubmitting, mutate, queryClient, user) => {
  mutate(values, {
    onSuccess: (response) => {
      ...
      queryClient.invalidateQueries(["user", user.id, "alamat"]);
    },
    onError: (error) => {
...
function SimpegFormAlamat({ user }) {
  ...
  const {
    ...
    queryFn: ({ signal }) =>
      axios
        .get(`/api/simpeg/${user.id}/alamat`, { signal })
        .then((res) => res.data)
        .catch((err) => {
...
export default SimpegFormAlamat;
