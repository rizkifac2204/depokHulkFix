...
function FormLaporBukti({ detail }) {
  ...
  const handleUpload = (e) => {
    ...
    setIsUploading(true);
    axios
      .post(`/api/lapor/${detail.id}/upload`, formData, {
        headers: {
      ...
      .then((res) => {
        queryClient.invalidateQueries({ queryKey: ["lapor", detail.id] });
        setKeterangan("");
  ...
  const deleteFile = (item) => {
    ...
    if (ask) {
      ...
      axios
        .delete(`/api/lapor/${detail.id}/upload`, {
          params: {
            id: item.id,
            file: item.file,
            path: "lapor",
