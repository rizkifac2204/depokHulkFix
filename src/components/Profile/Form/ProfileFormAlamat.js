...
const useHandleSubmitMutation = () => {
  return useMutation(async (formPayload) => {
    try {
      const res = await axios.put(`/api/profile/alamat`, formPayload);
      return res.data;
    } catch (err) {
...
const handleSubmit = (values, setSubmitting, mutate, queryClient) => {
  mutate(values, {
    onSuccess: (response) => {
      ...
      queryClient.invalidateQueries(["profile", "alamat"]);
    },
    onError: (error) => {
...
export default ProfileFormAlamat;
