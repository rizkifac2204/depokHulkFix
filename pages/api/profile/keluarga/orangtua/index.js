...
export default Handler()
  .get(async (req, res) => {
    try {
      const data = await db("user_keluarga_orangtua")
  ...
  .post(async (req, res) => {
    try {
      ...
      // proses simpan
      const proses = await db("user_keluarga_orangtua").insert(dataForInsert);
  ...
  .delete(async (req, res) => {
    try {
      ...
      const proses = await db("user_keluarga_orangtua")
        .whereIn("id", arrID)
        .del();
