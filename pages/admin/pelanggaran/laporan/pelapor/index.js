...
async function deleteData(id) {
  if (id) {
    try {
      const res = await axios.delete(`/api/pelanggaran/laporan/pelapor/${id}`);
      return res.data;
    } catch (err) {
...
function Pelapor() {
  ...
  const {
    ...
    queryFn: ({ signal }) =>
      axios
        .get(`/api/pelanggaran/laporan/pelapor`, { signal })
        .then((res) => res.data)
        .catch((err) => {
      ...
      getActions: (values) => {
        return [
          <GridActionsCellItem
            ...
            label="Detail"
            onClick={() =>
              router.push(`/admin/pelanggaran/laporan/pelapor/${values.id}`)
            }
          />,
          <GridActionsCellItem
            key={1}
            ...
            onClick={() =>
              router.push(
                `/admin/pelanggaran/laporan/pelapor/${values.id}/edit`
              )
            }
          />,
          <GridActionsCellItem
  ...
  return (
    <>
      <Head>
        <title>{`Data Pelapor - Bawaslu Depok Apps`}</title>
      </Head>
