...
async function postData(values) {
  ...
  if (!isNew) {
    try {
      const res = await axios.put(`/api/profile/riwayat/kursus/${id}`, values);
      return res.data;
    } catch (err) {
    ...
    try {
      const res = await axios.post(`/api/profile/riwayat/kursus`, values);
      return res.data;
    } catch (err) {
      throw new Error(err?.response?.data?.message || "Terjadi Kesalahan");
    }
...
async function deleteData(id) {
  if (id) {
    try {
      const res = await axios.delete(`/api/profile/riwayat/kursus/${id}`);
      return res.data;
    } catch (err) {
      throw new Error(err?.response?.data?.message || "Terjadi Kesalahan");
    }
...
function ProfileKursus() {
  const { data, isLoading } = useQuery({
    initialData: [],
    queryKey: ["profile", "riwayat", "kursus"],
    queryFn: ({ signal }) =>
      axios
        .get(`/api/profile/riwayat/kursus`, { signal })
        .then((res) => res.data)
        .catch((err) => {
  ...
  const { mutate, isLoading: isLoadingMutate } = useMutation({
    ...
    onSuccess: (data, variable, context) => {
      ...
      queryClient.invalidateQueries(["profile", "riwayat", "kursus"]);
    },
